...
class shippings extends CI_Controller
{
    ...
    function index(){
        ...
        $this->pagination_config['base_url']            = base_url('admins/shippings');
    ...
    function states(){
        $this->pagination_config['base_url']            = base_url('admins/shippings/states');
        $page                                           = $this->input->get('page') ? $this->input->get('page')-1 : 0 ;
        $country_id                                     = $this->input->get('sort_country'); 
        $state                                          = $this->input->get('sort_state');
        if($state || $country_id){
            $this->pagination_config['base_url']            = base_url('admins/shippings/states?sort_state='.$state.'&sort_country='.$country_id); 
        }
    ...
    function add_state(){
        ...
        redirect('admins/shippings/states');
    ...
    function cities(){
        $this->pagination_config['base_url']            = base_url('admins/shippings/states');
        $page                                           = $this->input->get('page') ? $this->input->get('page')-1 : 0 ;
        $state_id                                       = $this->input->get('sort_state'); 
        $city                                           = $this->input->get('sort_city');
        if($city || $state_id){
            $this->pagination_config['base_url']            = base_url('admins/shippings/cities?sort_city='.$city.'&sort_state='.$state_id); 
        }
        $data['CITIES']                                 = $this->admin_setup_model->GET_CITIES($this->pagination_config['per_page'], $page,$state_id,$city);
        $this->pagination_config['total_rows']          = $this->admin_setup_model->GET_TOTAL_CITY_COUNT($state_id,$city);
        $data['STATES']                                 = $this->admin_setup_model->GET_STATES_LIST();
        $data['STATE_ID']                               = $state_id;
        // Initialize the pagination library
        $this->pagination->initialize($this->pagination_config);
        $data['links']                                  = $this->pagination->create_links();
        $this->template->load('templates/admin_layout', 'modules/admin/shipping_city_views',$data);
    }
    function add_city(){
        $input_data=$this->input->post();
        $this->load->helper(array('form', 'url'));
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $input_data['created_at']=date('Y-m-d H:i:s');
            $input_data['updated_at']=date('Y-m-d H:i:s');
            $res=$this->admin_setup_model->ADD_ROW_DATA('tbl_cities',$input_data);
            if($res){
                $this->session->set_flashdata('alert',array('message'=>'New City added successfully','level'=>'success'));
            
            }else{
                $this->session->set_flashdata('alert',array('message'=>'Unable to add new city','level'=>'error'));
            }
        }
        redirect('admins/shippings/cities');
